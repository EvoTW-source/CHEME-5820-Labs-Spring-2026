...
To verify that IJulia is working correctly, navigate to this lab directory and run the notebook:
1. In VSCode, open a new terminal (or use the existing one)
2. Navigate to this directory: `cd labs/week-1/L1b` (adjust the path as needed based on where you cloned the repository)
3. Start Jupyter by typing `jupyter notebook` or `jupyter lab`
4. In the Jupyter interface that opens in your browser, open the notebook file in this directory
5. Try running a cell to confirm that the Julia kernel works properly

If you don't see Julia listed as an available kernel, you may need to rebuild IJulia by starting Julia again, entering package mode with `]`, and running `build IJulia`.

Once everything is installed, you're ready to start working with the course materials! When you clone lab repositories from GitHub Classroom, you'll see a `Project.toml` file that specifies the package dependencies needed for that lab. To set up your environment, navigate to the lab folder in your terminal, start Julia, enter package mode with `]`, and type `activate .` to activate the local environment. Then run the `Include.jl` file (by typing `include("Include.jl")` after exiting package mode) to download and set up all required packages for the assignment.
